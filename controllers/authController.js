...
const createSendToken = async (user, statusCode, res) => {
  ...
  console.log("Thời gian tạo token:", new Date());
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 2) Kiểm tra định dạng token cơ bản
  if (typeof token !== "string" || token.split(".").length !== 3) {
    console.error("Token không đúng định dạng:", token);
    return next(new AppError("Token không hợp lệ", 401));
  }

  // 3) Verify token
  try {
    // Sử dụng jwt.verify trực tiếp thay vì promisify
    ...
    // 5) Kiểm tra thay đổi mật khẩu
    if (currentUser.changedPasswordAfter(decoded.iat)) {
      return next(
        new AppError("Vui lòng đăng nhập lại do thay đổi mật khẩu", 401)
      );
    }
    ...
    // Xử lý cụ thể từng loại lỗi JWT
